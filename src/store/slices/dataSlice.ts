...
import { createOctolinearPath, createSmartOctolinearPath, snapBendToOctolinear } from '../../geometry/octolinear'
...
/** Keep moved endpoints and their adjoining bends on valid octolinear edges. */
function routeMovedSegment(segment: Segment, stations: Record<string, Station>, freeform: boolean): Point[] {
    const from = stations[segment.fromStationId]
    const to = stations[segment.toStationId]
    if (!from || !to) return segment.points
    const points = [...segment.points]
    const fromMoved = points[0].x !== from.x || points[0].y !== from.y
    const last = points.length - 1
    const toMoved = points[last].x !== to.x || points[last].y !== to.y
    if (!fromMoved && !toMoved) return points
    points[0] = { x: from.x, y: from.y }
    points[last] = { x: to.x, y: to.y }
    if (freeform) return points
    if (points.length === 2 || isAutoGeneratedLShape(segment)) {
        const obstacles = Object.values(stations)
            .filter((station) => station.id !== from.id && station.id !== to.id)
        return createSmartOctolinearPath(points[0], points[last], obstacles)
    }
    if (fromMoved) points[1] = snapBendToOctolinear(points[0], points[2], points[1])
    if (toMoved) points[last - 1] = snapBendToOctolinear(points[last - 2], points[last], points[last - 1])
    return points
}
            ...
            const segments = Object.fromEntries(
                Object.entries(state.segments).map(
                    ([segmentId, segment]) => {
                        ...
                        const newPoints = routeMovedSegment(
                            segment, { ...state.stations, [id]: newStation }, state.freeformMode,
                        )
        ...
        set((state) => {
            ...
            const segments = Object.fromEntries(
                Object.entries(state.segments).map(([id, segment]) => {
                    ...
                    const points = routeMovedSegment(segment, stations, state.freeformMode)
